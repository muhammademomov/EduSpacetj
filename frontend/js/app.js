// ═══════════════════════════════════════════════════════
// EduSpace.tj — Frontend App v2.2 (email verification)
// ═══════════════════════════════════════════════════════
...
function catSearchKey(e) { if (e.key === 'Enter') catSearch(e.target.value.trim()); }
...
// ═══════════════════════════════════════════════════════
// AUTH — REGISTER (ИЗМЕНЕНО: email вместо SMS)
// ═══════════════════════════════════════════════════════
...
// ✅ ИЗМЕНЕНО: отправляем регистрацию и показываем поле для email-кода
async function confirmTerms() {
    ...
    try {
        // Отправляем регистрацию на бэкенд
        const result = await post('/auth/register', regData);
        regData._userId = result.userId;
        ...
        // Показываем EMAIL вместо телефона
        document.getElementById('sms-ph').textContent = regData.email;
...
// ✅ ИЗМЕНЕНО: 6 полей вместо 4
function smsIn(el, idx) {
    el.classList.toggle('filled', el.value !== '');
    if (el.value && idx < 5) document.getElementById('s' + (idx + 1)).focus();
    const code = [0,1,2,3,4,5].map(i => {
        var s = document.getElementById('s' + i);
        return s ? s.value : '';
    }).join('');
    if (code.length === 6) setTimeout(verifySMS, 200);
}

// ✅ ИЗМЕНЕНО: проверяем код через /auth/verify-email
async function verifySMS() {
    ...
    if (code.length < 6) {
        ...
        document.getElementById('sms-err').textContent = 'Введите 6-значный код из письма';
        return;
    }
    ...
    try {
        const result = await post('/auth/verify-email', {
            userId: regData._userId,
            code: code
        });
        ...
        document.getElementById('sms-err').style.display = 'block';
        document.getElementById('sms-err').textContent = e.message || 'Неверный код';
    }
}
...
// ✅ ИЗМЕНЕНО: показываем кнопку повтора после истечения таймера
function startTimer() {
    regSec = 59; clearInterval(regTimer);
    document.getElementById('tmr-txt').textContent = '0:59';
    // Скрываем кнопку повтора пока идёт таймер
    ...
    regTimer = setInterval(() => {
        ...
        if (regSec <= 0) {
            clearInterval(regTimer);
            document.getElementById('tmr-txt').textContent = '';
            var resendBtn = document.getElementById('resend-code-btn');
...
// ✅ НОВАЯ ФУНКЦИЯ: отправить код повторно
async function resendCode() {
    if (!regData._userId) return;
    try {
        await post('/auth/resend-code', { userId: regData._userId });
        startTimer();
        document.getElementById('sms-err').style.display = 'none';
        showToast('Новый код отправлен на ' + regData.email);
    } catch(e) {
...
function afterReg() {
    ...
    [0,1,2,3,4,5].forEach(i => { const s=document.getElementById('s'+i); if(s){s.value='';s.classList.remove('filled');} });
    document.getElementById('rs1').style.display = 'block';
    document.getElementById('rs2').style.display = 'none';
...
async function doLogin() {
    ...
    try {
        ...
        // ✅ Если email не подтверждён — предлагаем ввести код
        if (e.status === 403 && e.message && e.message.includes('не подтверждён')) {
            document.getElementById('l-err').style.display = 'block';
            document.getElementById('l-err').textContent = '⚠️ Email не подтверждён. Проверьте почту или зарегистрируйтесь снова.';
        } else {
            document.getElementById('l-err').style.display = 'block';
...
function renderProfile(t) {
    ...
    document.getElementById('pp-docs-list').innerHTML = (t.documents||[]).length
        ? t.documents.map(function(d) {
            ...
            return '<' + tag + href + ' style="display:flex;align-items:center;gap:14px;padding:14px 16px;background:var(--bg);border-radius:10px;margin-bottom:8px;border:1.5px solid ' + (d.fileUrl ? 'rgba(37,99,235,.2)' : 'var(--border)') + ';text-decoration:none;color:inherit;">' +
    ...
    try {
        ...
        el.innerHTML = sorted.map(n => {
            ...
            if (n.type === 'review_comment' && n.link) {
                clickHandler = ' onclick="goToReviewTab(\'' + n.link + '\')" style="cursor:pointer"';
            }
                ...
                if (infoEl) {
                    ...
                    infoEl.innerHTML =
                        '<div style="background:#f0fdf4;border:1.5px solid var(--g);border-radius:12px;padding:14px;margin-bottom:14px">' +
                            ...
                            : '') +
                            '<div style="font-size:12px;color:var(--text3);margin-top:8px">✅ После одобрения курс будет куплен автоматически.</div>' +
                        '</div>';
        ...
        if (previewEl) {
            if (courses.length > 0) {
                previewEl.innerHTML = courses.slice(0,3).map(function(c) {
                    return '<div class="d-cr-row" onclick="openTeacherCourse(\'' + c.id + '\')" style="cursor:pointer">' +
                        '<div class="d-cr-ico">' + (c.emoji||'📖') + '</div>' +
                        '<div class="d-cr-inf">' +
                            '<div class="d-cr-t">' + c.title + '</div>' +
                            '<div class="d-cr-m">' + c.category + ' · ' + (c.student_count||0) + ' уч.</div>' +
                        '</div>' +
                    '</div>';
                }).join('');
            } else {
...
async function saveNewLesson() {
    ...
    try {
        await post('/teachers/lessons', { courseId: currentCourseId, title, content: desc });
        var modal = document.getElementById('add-lesson-modal');
...
function previewMatFile(input) {
    ...
    if (titleEl && !titleEl.value) titleEl.value = input.files[0].name.replace(/\.[^.]+$/, '');
}
...
async function showCourseEditModal(courseId) {
    try {
        ...
        modal.innerHTML =
            '<div style="background:var(--white);border-radius:20px;width:100%;max-width:520px;padding:1.5rem;max-height:90vh;overflow-y:auto">' +
                '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem">' +
    ...
    try {
        ...
        el.innerHTML = '<div class="d-card" style="padding:1.2rem">' + students.map(s => {
            ...
            var av  = s.avatar_url
                ...
                : (s.initials || '?');
            return '<div class="d-st-row" onclick="openStudentPage(\'' + s.id + '\', \'' + s.course_id + '\')" style="cursor:pointer">'+
                ...
                '<div style="flex:1">'+
                    ...
                    '<div style="font-size:11px;color:var(--text2)">' + (s.emoji||'📖') + ' ' + s.course_title + '</div>'+
                '</div>'+
...
async function saveTeacherProfile() {
    try {
        ...
        localStorage.setItem('user', JSON.stringify(currentUser));
        showLoggedIn();
        var saveBtn2 = document.querySelector('button.btn-save');
        if (saveBtn2) { saveBtn2.textContent = 'Сохранить изменения'; saveBtn2.disabled = false; }
        showToast('Профиль сохранён');
    } catch(e) { alert('Ошибка: ' + e.message); }
}
...
async function loadStudentChats() {
    ...
    try {
        ...
        if (badge) { badge.textContent = '0'; badge.style.display = 'none'; }
        if (!chats.length) {
            el.innerHTML = '<div class="empty-state"><div class="empty-icon">💬</div><div class="empty-title">Нет переписок</div><button class="btn-lg green" onclick="go(\'catalog\')">Найти преподавателя</button></div>';
            return;
        }
        el.innerHTML = chats.map(function(c) {
            ...
            return '<div class="chat-list-item" onclick="openChatWithStudent(\'' + c.id + '\', \'' + safeName + '\', \'' + initials + '\', \'' + safeColor + '\', \'' + (c.avatar_url||'') + '\')">' +
                ...
                '</div>' +
                '<div class="cli-info"><div class="cli-name">' + c.first_name + ' ' + c.last_name + '</div><div class="cli-last">' + shortMsg + '</div></div>' +
    ...
    } catch(e) { console.error('loadStudentChats:', e); }
}
...
async function loadTeacherChats() {
    ...
    try {
        const chats = await get('/users/chats');
        if (!chats.length) {
            el.innerHTML = '<div class="empty-state"><div class="empty-icon">💬</div><div class="empty-title">Нет сообщений</div></div>';
            return;
        }
...
async function loadTeacherNotifications() {
    ...
    try {
        ...
        const sorted = [...notifs].sort((a, b) => a.is_read - b.is_read);
        const ICONS  = { new_message:'💬', homework:'📝', new_material:'📎', topup:'💳', welcome:'🎉', new_teacher:'👤', new_course:'📚', review_comment:'⭐' };
...
async function submitCourse() {
    ...
    const lessons = Array.from(document.querySelectorAll('#ac-lessons .l-title')).map(el => el.textContent);
    try {
        await post('/courses', { title, category: cat, level: document.getElementById('ac-lvl').value, description: document.getElementById('ac-desc').value, price, lessons });
...
function buildTccard(t) {
    ...
    return '<div class="tc2card' + (t.isModerated?' verified':'') + '" onclick="openProfile(\'' + t.id + '\')">' +
        '<div class="tc2-photo-wrap">' + av + (t.isModerated ? '<div class="tc2-check">✓</div>' : '') + '</div>' +
        '<div class="tc2-body">' +
            '<div class="tc2-top">' +
                '<div><div class="tc2-name">' + (t.firstName||t.first_name||'') + ' ' + (t.lastName||t.last_name||'') + '</div><div class="tc2-subj">' + (t.subject||'Предмет не указан') + '</div></div>' +
...
function buildCcard(c) {
    return `<div class="ccard" onclick="openProfile('${c.teacher?.id||''}')">
...
function setAvatar(el, user) {
    ...
    if (user.avatarUrl) {
        ...
        img.onerror = function() { el.innerHTML = user.initials || '?'; el.style.padding = ''; el.style.overflow = ''; };
        el.innerHTML = '';
        el.appendChild(img);
    } else {
...
async function uploadStudentPhoto(input) {
    ...
    reader.onload = function(e) {
        ...
        if (avEl) { avEl.style.padding = '0'; avEl.style.overflow = 'hidden'; avEl.innerHTML = '<img src="' + e.target.result + '" style="width:100%;height:100%;object-fit:cover;border-radius:inherit">'; }
    };
...
function previewVideo(input) {
    ...
    var preview = document.getElementById('tp-video-preview');
    var player = document.getElementById('tp-video-player');
    if (preview && player) { player.src = url; preview.style.display = 'block'; }
}
...
async function uploadTeacherVideo() {
    ...
    } catch(e) { alert('Ошибка загрузки: ' + e.message); btn.textContent = 'Загрузить'; btn.disabled = false; }
}
...
async function loadTeacherDocs() {
    ...
    try {
        ...
        if (!docs.length) { el.innerHTML = '<div style="color:var(--text3);font-size:13px;padding:12px;background:var(--bg);border-radius:9px">Документы не загружены</div>'; return; }
...
async function addTeacherDoc() {
    ...
    btn.textContent = '+ Добавить документ'; btn.disabled = false;
}
...
function openChat() {
    if (!currentUser) { go('login'); return; }
    if (!currentProfileId) return;
    if (currentUser.role === 'teacher' && currentProfileId === currentUser.id) { alert('Вы не можете написать самому себе'); return; }
    ...
    if (avEl) { var ppAv = document.getElementById('pp-av'); if (ppAv) avEl.innerHTML = ppAv.innerHTML || ppAv.textContent; }
    var modal = document.getElementById('chat-modal');
    if (!modal) return;
    modal.style.display = 'flex';
    document.body.style.overflow = 'hidden';
...
async function loadMessages() {
    ...
    try {
        ...
        if (!msgs.length) { el.innerHTML = '<div class="chat-empty"><div class="chat-empty-ico">💬</div>Начните общение с преподавателем</div>'; return; }
        var html = msgs.map(function(m) {
            ...
            return '<div class="chat-msg ' + (isMe ? 'me' : 'them') + '"><div class="chat-bubble">' + safeText + '</div><div class="chat-time">' + time + '</div></div>';
        }).join('');
        el.innerHTML = html;
...
async function sendMsg(e) {
    ...
    try {
        ...
        await loadMessages();
    } catch(ex) { input.value = text; showToast('Ошибка отправки: ' + ex.message, 'error'); }
    finally { if (sendBtn) sendBtn.style.opacity = '1'; }
}
...
function showToast(msg, type) {
    ...
    setTimeout(function() { t.style.opacity = '0'; t.style.transform = 'translateX(-50%) translateY(10px)'; setTimeout(function() { t.remove(); }, 300); }, 3200);
}
...
async function loadTeacherReviews() {
    ...
    try {
        ...
        if (!reviews.length) {
            el.innerHTML = '<div style="text-align:center;padding:3rem;color:var(--text3)"><div style="font-size:2rem;margin-bottom:.5rem">⭐</div><div style="font-weight:700">Отзывов пока нет</div></div>';
            return;
        }
        var newBanner = '';
        if (newCommentNotifs.length > 0) {
            newBanner = '<div style="display:flex;align-items:center;gap:.75rem;background:#f0fdf4;border:1.5px solid var(--g);border-radius:12px;padding:.875rem 1rem;margin-bottom:1rem">' +
                '<div style="font-size:22px">💬</div>' +
                '<div><div style="font-size:13px;font-weight:800;color:var(--g2)">Новые ответы (' + newCommentNotifs.length + ')</div></div>' +
            '</div>';
        }
            ...
            reviews.map(function(r) {
                ...
                return '<div style="padding-bottom:1.2rem;border-bottom:1px solid var(--border2)" id="rev-card-' + r.id + '">' +
                    '<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.5rem">' +
                        ...
                        '<div style="font-size:11px;color:var(--text3)">' + date + '</div></div>' +
    ...
    } catch(e) { console.error(e); el.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--text3)">Ошибка загрузки</div>'; }
}
    ...
    try {
        ...
        el.innerHTML = '<div class="rev-thread-wrap">' +
            comments.map(function(c) {
                ...
                var date = new Date(c.date).toLocaleDateString('ru', {day:'numeric', month:'short'});
                ...
                return '<div class="rev-comment' + (isTeacherComment ? ' rev-comment--teacher' : '') + '" id="rev-cmt-' + c.id + '">' +
                    ...
                    '<div class="rev-cmt-body">' +
                        '<div class="rev-cmt-meta"><span class="rev-cmt-name">' + c.author.name + '</span>' +
                        (isTeacherComment ? '<span class="rev-cmt-badge">Преподаватель</span>' : '') +
                        '<span class="rev-cmt-date">' + date + '</span></div>' +
...
async function submitReview() {
    ...
    try {
        ...
        try {
            ...
            var match = enrollments.find(function(e) { return e.teacherId === currentProfileId || e.teacher_id === currentProfileId; });
            if (match) courseId = match.courseId || match.course_id;
        } catch(e) {}
        await post('/users/reviews', { teacherId: currentProfileId, courseId: courseId || null, stars: selectedStars, text: text });
        selectedStars = 0; setStars(0);
        document.getElementById('pp-rev-text').value = '';
        document.getElementById('pp-rev-form').style.display = 'none';
        btn.textContent = 'Отправить отзыв'; btn.disabled = false;
        showToast('✅ Спасибо за отзыв!');
        openProfile(currentProfileId);
    } catch(e) { showToast('Ошибка: ' + (e.message || ''), 'error'); btn.textContent = 'Отправить отзыв'; btn.disabled = false; }
}
...
function openChatFromCourse() {
    ...
    if (btn) { var b = btn.querySelector('.chat-page-badge'); if (b) b.remove(); btn.style.borderColor = ''; btn.style.color = ''; }
    const nameEl = document.getElementById('chat-name');
    const avEl   = document.getElementById('chat-av');
    if (nameEl) nameEl.textContent = teacher.firstName + ' ' + teacher.lastName;
    if (avEl) {
        avEl.style.background = teacher.color; avEl.style.overflow = 'hidden';
        if (teacher.avatarUrl) { avEl.innerHTML = '<img src="' + teacher.avatarUrl + '" style="width:100%;height:100%;object-fit:cover;border-radius:inherit">'; }
...
async function loadCourseData() {
    try {
        ...
        if (e.status === 403) { showToast('Вы не записаны на этот курс', 'error'); go('student-dash'); loadStudentDash(); sdShow('my-courses'); }
        else { showToast('Ошибка загрузки курса: ' + (e.message || ''), 'error'); }
    }
}
...
function renderCoursePage(data) {
    ...
    if (tabsWrap) {
        if (isTeacherView) {
            tabsWrap.innerHTML = '<button class="cp-tab on" onclick="cpTab(\'lessons\', this)"><span class="tab-full">📚 Уроки</span><span class="tab-short">📚</span></button><button class="cp-tab" onclick="cpTab(\'materials\', this)"><span class="tab-full">📎 Материалы</span><span class="tab-short">📎</span></button>';
        } else {
            tabsWrap.innerHTML = '<button class="cp-tab on" onclick="cpTab(\'lessons\', this)"><span class="tab-full">📚 Уроки</span><span class="tab-short">📚</span></button><button class="cp-tab" onclick="cpTab(\'schedule\', this)"><span class="tab-full">📅 Расписание</span><span class="tab-short">📅</span></button><button class="cp-tab" onclick="cpTab(\'homework\', this)"><span class="tab-full">📝 Домашние задания</span><span class="tab-short">📝</span></button><button class="cp-tab" onclick="cpTab(\'materials\', this)"><span class="tab-full">📎 Материалы</span><span class="tab-short">📎</span></button>';
        }
    }
    ...
    if (isTeacherView && lessonsSection) {
        ...
        if (!oldAddBtn) {
            lessonsSection.insertAdjacentHTML('afterbegin',
                '<div id="cp-add-lesson-btn" style="margin-bottom:16px"><button onclick="showAddLessonModal()" style="width:100%;display:flex;align-items:center;justify-content:center;gap:8px;background:var(--gl2);border:2px dashed var(--g);border-radius:12px;padding:14px;font-size:14px;font-weight:700;color:var(--g2);cursor:pointer"><span style="font-size:20px">+</span> Добавить новый урок</button></div>'
            );
        }
    }
    ...
    if (avEl) {
        if (course.teacher.avatarUrl) { avEl.innerHTML = '<img src="' + course.teacher.avatarUrl + '" style="width:100%;height:100%;object-fit:cover">'; }
        else { avEl.textContent = course.teacher.initials || '?'; avEl.style.background = course.teacher.color || '#18A96A'; }
    }
...
function openLessonDetail(lessonId, e) {
    ...
    var TYPE_ICONS = {pdf:'📄',doc:'📝',docx:'📝',ppt:'📊',pptx:'📊',xls:'📈',xlsx:'📈',jpg:'🖼️',jpeg:'🖼️',png:'🖼️',gif:'🖼️',mp4:'🎬',mp3:'🎵',zip:'🗜️',txt:'📋'};
    ...
    if (lessonMats.length) {
        matsHtml = '<div style="margin-top:16px"><div style="font-size:12px;font-weight:800;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">📎 Файлы к уроку</div>' +
            lessonMats.map(function(m) {
                ...
                return '<a href="' + (m.fileUrl||'#') + '" target="_blank" style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--bg);border-radius:10px;text-decoration:none;color:inherit;margin-bottom:8px;border:1px solid var(--border2)">' +
                    ...
                    '<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + m.title + '</div><div style="font-size:11px;color:var(--text3)">' + (size ? size + ' · ' : '') + ext.toUpperCase() + '</div></div>' +
                    '<div style="font-size:16px;color:var(--g2)">↓</div></a>';
            }).join('') + '</div>';
    }
    modal.innerHTML =
        '<div style="background:var(--white);border-radius:20px;width:100%;max-width:560px;padding:1.5rem;max-height:85vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,.2)">' +
            '<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px">' +
                '<div><div style="font-size:11px;font-weight:700;color:var(--text3);margin-bottom:4px">Урок ' + lesson.order + '</div><div style="font-size:18px;font-weight:800">' + lesson.title + '</div></div>' +
            ...
            '</div>' +
            (lesson.content ? '<div style="padding:12px;background:var(--bg);border-radius:10px;margin-bottom:4px"><div style="font-size:14px;color:var(--text2);line-height:1.7;white-space:pre-wrap">' + lesson.content + '</div></div>' : '') +
            matsHtml +
...
function renderLessons(lessons) {
    ...
    if (!lessons.length) { el.innerHTML = '<div class="cp-empty"><div class="cp-empty-ico">📚</div><div class="cp-empty-title">Уроков пока нет</div></div>'; return; }
    var isT = currentUser && currentUser.role === 'teacher';
    var matsByLesson = {};
    if (window._currentCourseData && window._currentCourseData.materials) {
        window._currentCourseData.materials.forEach(function(m) {
            if (m.lessonId) { if (!matsByLesson[m.lessonId]) matsByLesson[m.lessonId] = []; matsByLesson[m.lessonId].push(m); }
        });
    }
    el.innerHTML = lessons.map(function(l) {
        ...
        return '<div class="cp-lesson' + (l.isDone && !isT ? ' done' : '') + '" id="lesson-' + l.id + '" onclick="openLessonDetail(\'' + l.id + '\', event)" style="cursor:pointer">' +
            ...
            '<div class="cp-lesson-info"><div class="cp-lesson-title">' + l.title + '</div>' +
            '<div class="cp-lesson-meta">' + (l.isDone && l.doneAt && !isT ? 'Пройдено ' + new Date(l.doneAt).toLocaleDateString('ru',{day:'numeric',month:'short'}) : 'Урок ' + l.order) + (hasMats ? ' · 📎 ' + lessonMats.length : '') + '</div></div>' +
            (isT ? '<div style="display:flex;align-items:center;gap:4px"><span style="font-size:12px;color:var(--text3)">→</span><button onclick="event.stopPropagation();deleteLesson(\'' + l.id + '\')" style="background:none;border:none;color:#EF4444;font-size:16px;cursor:pointer;padding:4px 6px;border-radius:6px;opacity:.7">🗑</button></div>'
            : '<div class="cp-lesson-check" onclick="event.stopPropagation();toggleLesson(\'' + l.id + '\', ' + (!l.isDone) + ', event)">' + (l.isDone ? '✓' : '') + '</div>') +
        '</div>';
    }).join('');
}
...
async function toggleLesson(lessonId, isDone, e) {
    ...
    try {
        ...
        showToast(isDone ? '✅ Урок пройден' : 'Урок отмечен не пройденным', isDone ? 'success' : 'info');
...
function renderSchedule(schedule, teacher) {
    ...
    if (schedule && schedule.length) {
        ...
        grid.innerHTML = '<div class="cp-sched-grid">' + schedule.map(function(s) {
            ...
            return '<div class="cp-sched-card"><div class="cp-sched-day">' + dayFull + '</div><div class="cp-sched-time">' + (s.timeFrom || '—') + (s.timeTo ? ' – ' + s.timeTo : '') + '</div><div class="cp-sched-plat">' + platIco + ' ' + (s.platform || '') + '</div>' + linkBtn + '</div>';
        }).join('') + '</div>';
    } else {
    ...
    if (teacher && (teacher.workDays.length || teacher.workHours)) {
        ...
        if (daysEl && teacher.workDays.length) { daysEl.innerHTML = '<div style="font-size:12px;color:var(--text2);margin-bottom:6px"><b>Рабочие дни:</b> ' + teacher.workDays.join(', ') + (teacher.workHours ? ' · ' + teacher.workHours : '') + '</div>'; }
...
function renderHomework(homework) {
    ...
    el.innerHTML = homework.map(function(h) {
        ...
        return '<div class="cp-hw-card ' + statusClass + '">' +
            '<div class="cp-hw-hdr"><div class="cp-hw-ico">📝</div><div style="flex:1"><div class="cp-hw-title">' + h.title + '</div><div class="cp-hw-meta">' + due + '</div></div><span class="cp-hw-status ' + statusClass + '">' + statusLabel + '</span></div>' +
            (h.description ? '<div class="cp-hw-desc">' + h.description + '</div>' : '') +
            (h.studentAnswer ? '<div class="cp-hw-answer">💬 Ваш ответ: ' + h.studentAnswer + '</div>' : '') +
            (h.teacherComment ? '<div class="cp-hw-comment">👨‍🏫 Комментарий: ' + h.teacherComment + '</div>' : '') +
            submitBtn +
        '</div>';
    }).join('');
}
...
function renderMaterials(materials) {
    ...
    if (matsSection && isT) {
        ...
        if (!oldBtn) {
            matsSection.insertAdjacentHTML('afterbegin',
                '<div id="cp-upload-mat-btn" style="margin-bottom:16px"><div style="background:var(--white);border:1.5px solid var(--border);border-radius:14px;padding:16px">' +
                '<div style="font-size:14px;font-weight:800;margin-bottom:12px">➕ Добавить материал</div>' +
                '<div style="display:flex;gap:8px;margin-bottom:14px"><button id="mat-type-file" onclick="setMatType(\'file\')" style="flex:1;padding:9px;border:1.5px solid var(--g);border-radius:9px;background:var(--g);color:#fff;font-size:12px;font-weight:700;cursor:pointer">📎 Файл</button><button id="mat-type-link" onclick="setMatType(\'link\')" style="flex:1;padding:9px;border:1.5px solid var(--border);border-radius:9px;background:none;font-size:12px;font-weight:700;cursor:pointer;color:var(--text2)">🔗 Ссылка</button></div>' +
                ...
                '<div id="mat-field-file"><label style="display:flex;align-items:center;gap:10px;background:var(--bg);border:1.5px dashed var(--border);border-radius:9px;padding:12px;cursor:pointer"><span style="font-size:22px">📁</span><div><div style="font-size:13px;font-weight:700" id="mat-fname">Выбрать файл</div></div><input type="file" id="cp-mat-file-input" style="display:none" onchange="previewMatFile(this)"></label></div>' +
                '<div id="mat-field-link" style="display:none"><input id="mat-url" type="url" placeholder="https://..." style="width:100%;padding:10px 12px;border:1.5px solid var(--border);border-radius:9px;font-size:13px;outline:none;box-sizing:border-box"></div>' +
    ...
    if (!materials || !materials.length) { if (empty) empty.style.display = isT ? 'none' : ''; if (el) el.innerHTML = ''; return; }
    if (empty) empty.style.display = 'none';
    const TYPE_ICONS = {pdf:'📄',doc:'📝',docx:'📝',ppt:'📊',pptx:'📊',xls:'📈',xlsx:'📈',jpg:'🖼️',jpeg:'🖼️',png:'🖼️',mp4:'🎬',mp3:'🎵',zip:'🗜️'};
    el.innerHTML = materials.map(function(m) {
        var ext = m.fileType || (m.fileUrl ? m.fileUrl.split('?')[0].split('.').pop().toLowerCase() : '');
        var ico  = TYPE_ICONS[ext] || '📎';
        var size = m.fileSize ? (m.fileSize > 1024*1024 ? (m.fileSize/1024/1024).toFixed(1)+' МБ' : (m.fileSize/1024).toFixed(0)+' КБ') : '';
        var date = m.createdAt ? new Date(m.createdAt).toLocaleDateString('ru',{day:'numeric',month:'short'}) : '';
        ...
        return '<div style="display:flex;align-items:center;gap:8px">' +
            ...
            '<div class="cp-mat-info"><div class="cp-mat-title">' + m.title + '</div><div class="cp-mat-meta">' + (size ? size + ' · ' : '') + date + '</div></div>' +
            (m.fileUrl ? '<div class="cp-mat-dl">⬇️ Скачать</div>' : '') +
            '</' + tag + '>' +
            (isTeach ? '<button onclick="deleteCourseMaterial(\'' + m.id + '\')" style="background:none;border:none;color:#EF4444;font-size:18px;cursor:pointer;padding:8px;flex-shrink:0;opacity:.7">🗑</button>' : '') +
        '</div>';
    }).join('');
}
...
function previewLessonFile(input) {
    ...
    if (preview) { preview.style.display = 'flex'; document.getElementById('al-file-ico').textContent = icons[ext] || '📎'; document.getElementById('al-file-info').textContent = file.name + ' · ' + size; }
}

function clearLessonFile() {
    var input = document.getElementById('al-file'); if (input) input.value = '';
    var label = document.getElementById('al-file-label'); var preview = document.getElementById('al-file-preview');
    if (label) label.style.display = 'flex'; if (preview) preview.style.display = 'none';
}

function previewMatFile2(input) {
    if (!input.files || !input.files[0]) return;
    var name = input.files[0].name;
    document.getElementById('ts-mat-fname').textContent = name;
}
...
async function uploadMaterial() {
    ...
    try {
        ...
        var res = await fetch(API + '/teachers/materials/upload', { method: 'POST', headers: token ? { 'Authorization': 'Bearer ' + token } : {}, body: fd });
        ...
        showToast('✅ Материал загружен!');
        await loadStudentPageData();
    } catch(e) { showToast('Ошибка: ' + (e.message || ''), 'error'); }
    finally { btn.textContent = '⬆️ Загрузить материал'; btn.disabled = false; }
}

function renderTsMaterials(materials) {
    var el = document.getElementById('ts-mat-list');
    if (!el) return;
    if (!materials || !materials.length) { el.innerHTML = '<div class="cp-empty"><div class="cp-empty-ico">📎</div><div class="cp-empty-title">Материалов пока нет</div></div>'; return; }
    var TYPE_ICONS = {pdf:'📄',doc:'📝',docx:'📝',ppt:'📊',pptx:'📊',xls:'📈',xlsx:'📈',jpg:'🖼️',jpeg:'🖼️',png:'🖼️',mp4:'🎬',mp3:'🎵',zip:'🗜️'};
    el.innerHTML = materials.map(function(m) {
        ...
        return '<div class="cp-mat-card" style="display:flex;align-items:center;gap:14px">' +
            '<div class="cp-mat-ico">' + ico + '</div>' +
            '<div class="cp-mat-info" style="flex:1"><div class="cp-mat-title">' + m.title + '</div><div class="cp-mat-meta">' + size + '</div></div>' +
            (m.fileUrl ? '<a href="' + m.fileUrl + '" target="_blank" class="btn-sm ghost">⬇️ Открыть</a>' : '') +
            '<button onclick="deleteMaterial(\'' + m.id + '\')" style="background:none;border:none;color:var(--text3);font-size:18px;cursor:pointer;padding:4px">🗑</button>' +
        '</div>';
    }).join('');
}
...
async function deleteMaterial(matId) {
    ...
    try {
        var token = localStorage.getItem('token');
        var res = await fetch(API + '/teachers/materials/' + matId, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } });
...
function renderStudentPage(data) {
    ...
    document.getElementById('ts-enrolled-at').textContent  = 'Записан ' + new Date(enrollment.enrolledAt).toLocaleDateString('ru',{day:'numeric',month:'short',year:'numeric'});
    const avEl = document.getElementById('ts-av');
    if (student.avatarUrl) { avEl.innerHTML = '<img src="' + student.avatarUrl + '" style="width:100%;height:100%;object-fit:cover;border-radius:16px">'; avEl.style.background = student.color || '#18A96A'; }
    else { avEl.textContent = student.initials || '?'; avEl.style.background = student.color || '#18A96A'; avEl.style.color = '#fff'; avEl.style.display = 'flex'; avEl.style.alignItems = 'center'; avEl.style.justifyContent = 'center'; avEl.style.fontWeight = '800'; }
    const pct = progress.percent || 0;
    const offset = 201 - (201 * pct / 100);
    ...
    if (lessons.length) {
        lessonsEl.innerHTML = lessons.map(function(l) {
            return '<div class="cp-lesson' + (l.isDone ? ' done' : '') + '">' +
                ...
                '<div class="cp-lesson-info"><div class="cp-lesson-title">' + l.title + '</div><div class="cp-lesson-meta">' + (l.isDone ? '✅ Пройдено' : '⏳ Не пройдено') + '</div></div>' +
                '<div style="font-size:20px">' + (l.isDone ? '✅' : '⬜') + '</div>' +
            '</div>';
        }).join('');
    } else { lessonsEl.innerHTML = '<div class="cp-empty"><div class="cp-empty-ico">📚</div><div class="cp-empty-title">Уроков нет</div></div>'; }
    const sel = document.getElementById('ts-hw-lesson');
    sel.innerHTML = '<option value="">— Общее задание —</option>' + lessons.map(function(l) { return '<option value="' + l.id + '">Урок ' + l.order + ': ' + l.title + '</option>'; }).join('');
    renderTsSchedule(schedule);
    renderTsHomework(homework);
    const selMat = document.getElementById('ts-mat-lesson');
    if (selMat) { selMat.innerHTML = '<option value="">— Общий материал —</option>' + lessons.map(function(l) { return '<option value="' + l.id + '">Урок ' + l.order + ': ' + l.title + '</option>'; }).join(''); }
...
function addSchedSlot() {
    ...
    div.innerHTML = '<button onclick="this.parentElement.remove()" style="position:absolute;right:0;top:14px;background:none;border:none;color:#EF4444;font-size:18px;cursor:pointer">✕</button>' +
        '<div class="field-row"><div class="field"><label>День</label><select class="ts-day" style="width:100%;padding:10px 14px;border:1.5px solid var(--border);border-radius:9px;font-size:14px">' +
        '<option value="Пн">Понедельник</option><option value="Вт">Вторник</option><option value="Ср">Среда</option><option value="Чт">Четверг</option><option value="Пт">Пятница</option><option value="Сб">Суббота</option><option value="Вс">Воскресенье</option>' +
        '</select></div><div class="field"><label>Начало</label><input type="time" class="ts-from" value="09:00" style="width:100%;padding:10px 14px;border:1.5px solid var(--border);border-radius:9px;font-size:14px"></div>' +
        '<div class="field"><label>Конец</label><input type="time" class="ts-to" value="10:00" style="width:100%;padding:10px 14px;border:1.5px solid var(--border);border-radius:9px;font-size:14px"></div></div>' +
        '<div class="field"><label>Ссылка</label><input type="url" class="ts-link" placeholder="https://zoom.us/j/..." style="width:100%;padding:10px 14px;border:1.5px solid var(--border);border-radius:9px;font-size:14px"></div>';
...
async function saveSchedule() {
    ...
    const days = Array.from(slots).map(function(s) {
        return { dayOfWeek: s.querySelector('.ts-day')?.value || '', timeFrom: s.querySelector('.ts-from')?.value || '', timeTo: s.querySelector('.ts-to')?.value || '', link: s.querySelector('.ts-link')?.value || '' };
    });
    try {
        await post('/courses/' + currentCourseId + '/schedule', { enrollmentId: currentStudentData.enrollment.id, studentId: currentStudentId, days });
...
function renderTsSchedule(schedule) {
    ...
    const DAY_NAMES = {'Пн':'Понедельник','Вт':'Вторник','Ср':'Среда','Чт':'Четверг','Пт':'Пятница','Сб':'Суббота','Вс':'Воскресенье'};
    el.innerHTML = '<div class="cp-sched-grid">' + schedule.map(function(s) {
        var dayFull = DAY_NAMES[s.dayOfWeek] || s.dayOfWeek || '—';
        var linkBtn = s.link ? '<a href="' + s.link + '" target="_blank" class="cp-sched-link">🔗 Войти</a>' : '';
        return '<div class="cp-sched-card"><div class="cp-sched-day">' + dayFull + '</div><div class="cp-sched-time">' + (s.timeFrom||'—') + (s.timeTo ? ' – ' + s.timeTo : '') + '</div>' + linkBtn + '</div>';
    }).join('') + '</div>';
}
...
async function addHomework() {
    ...
    try {
        await post('/teachers/homework', { courseId: currentCourseId, studentId: currentStudentId, lessonId: lessonId || null, title, description: desc, dueDate: due || null });
...
function renderTsHomework(homework) {
    ...
    if (!homework || !homework.length) { el.innerHTML = '<div class="cp-empty"><div class="cp-empty-ico">📝</div><div class="cp-empty-title">Заданий пока нет</div></div>'; return; }
    const STATUS_LABELS = { pending:'📋 Ожидает', submitted:'✅ Сдано', reviewed:'💬 Проверено' };
    el.innerHTML = homework.map(function(h) {
        var statusLabel = STATUS_LABELS[h.status] || h.status;
        var reviewBtn = h.status === 'submitted' ? '<button class="btn-sm solid" onclick="openTsComment(\'' + h.id + '\', \'' + (h.studentAnswer||'').replace(/'/g,"\\'") + '\')">💬 Проверить</button>' : '';
        return '<div class="cp-hw-card ' + (h.status||'pending') + '" style="margin-bottom:10px">' +
            '<div class="cp-hw-hdr"><div class="cp-hw-ico">📝</div><div style="flex:1"><div class="cp-hw-title">' + h.title + '</div></div><span class="cp-hw-status">' + statusLabel + '</span></div>' +
            (h.studentAnswer ? '<div class="cp-hw-answer">💬 Ответ: ' + h.studentAnswer + '</div>' : '') +
            (h.teacherComment ? '<div class="cp-hw-comment">👨‍🏫 Комментарий: ' + h.teacherComment + '</div>' : '') +
            reviewBtn +
        '</div>';
    }).join('');
...
function startCourseChatPoll(otherUserId, btnId) {
    ...
    courseChatPollInterval = setInterval(function() { checkCourseChat(otherUserId, btnId); }, 5000);
}
...
async function checkCourseChat(otherUserId, btnId) {
    ...
    try {
        ...
        if (unread > 0) {
            ...
            badge.style.cssText = 'display:inline-flex;align-items:center;justify-content:center;background:#EF4444;color:#fff;border-radius:50%;width:18px;height:18px;font-size:11px;font-weight:800;margin-left:6px;';
...
function selectTopupMethod(method) {
    ...
    if (!alif) return;
    if (method === 'alif_mobi') {
        alif.style.border = '2px solid var(--g)'; card.style.border = '2px solid transparent';
        alifD.style.display = 'block'; cardD.style.display = 'none';
    } else {
        card.style.border = '2px solid var(--g)'; alif.style.border = '2px solid transparent';
        cardD.style.display = 'block'; alifD.style.display = 'none';
    }
}
...
async function submitTopupRequest() {
    ...
    try {
        await post('/payments/topup-request', { amount: parseFloat(amount), method: _topupMethod, transaction_id: txid, comment, course_id: pendingCourseId || null });
        ...
        if (payWrap) {
            payWrap.innerHTML = '<div style="text-align:center;padding:3rem 1rem"><div style="font-size:56px;margin-bottom:16px">⏳</div><div style="font-size:18px;font-weight:800;margin-bottom:8px">Заявка отправлена!</div><div style="font-size:14px;color:var(--text2);margin-bottom:24px;line-height:1.6">Администратор проверит перевод и одобрит.</div><div style="background:var(--bg);border-radius:12px;padding:14px;font-size:13px;color:var(--text3)">🔄 Страница обновится автоматически...</div></div>';
        }
        _startApprovalPolling();
    } catch(e) { errEl.textContent = e.message || 'Ошибка отправки'; errEl.style.display = 'block'; }
        ...
        el.innerHTML = '<div class="d-card"><div class="d-card-title">История заявок</div>' +
            reqs.map(function(r) {
                return '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border2)">' +
                    ...
                    '<div style="font-size:11px;color:var(--text3)">' + new Date(r.created_at).toLocaleDateString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}) + '</div></div>' +
                    '<span style="font-size:12px;font-weight:700;color:' + colorMap[r.status] + '">' + statusMap[r.status] + '</span>' +
                '</div>';
            }).join('') + '</div>' +
            (hasApproved ? '<button class="btn-full green" style="margin-top:12px" onclick="onBalanceApproved()">' + (pendingCourseId ? '🎓 Оплатить курс' : '🔍 Перейти в каталог') + '</button>' : '') +
            (hasPending && !hasApproved ? '<div style="text-align:center;font-size:12px;color:var(--text3);margin-top:10px">⏳ Ожидаем одобрения...</div>' : '');
    } catch(e) {}
}
...
async function loadTeacherEarnings() {
    ...
    try {
        ...
        if (!listEl) return;
        if (!payments.length) { listEl.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--text3)">Продаж пока нет</div>'; return; }
        var totalGrossInc = payments.filter(p=>p.type==='income').reduce((s,p)=>s+p.pricePaid,0);
        var totalCommInc  = payments.filter(p=>p.type==='income').reduce((s,p)=>s+p.commission,0);
        var totalNetInc   = payments.filter(p=>p.type==='income').reduce((s,p)=>s+p.amount,0);
        var totalWithdrawn= payments.filter(p=>p.type==='withdrawal').reduce((s,p)=>s+p.amount,0);
        listEl.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin-bottom:16px">' +
            '<div style="background:var(--bg);border-radius:10px;padding:10px;text-align:center"><div style="font-size:10px;color:var(--text3);margin-bottom:3px">Студенты заплатили</div><div style="font-size:15px;font-weight:800">' + totalGrossInc.toLocaleString('ru') + ' смн</div></div>' +
            '<div style="background:#fee2e2;border-radius:10px;padding:10px;text-align:center"><div style="font-size:10px;color:#991b1b;margin-bottom:3px">Комиссия</div><div style="font-size:15px;font-weight:800;color:#DC2626">−' + totalCommInc.toLocaleString('ru') + ' смн</div></div>' +
            '<div style="background:#d1fae5;border-radius:10px;padding:10px;text-align:center"><div style="font-size:10px;color:#065f46;margin-bottom:3px">Начислено</div><div style="font-size:15px;font-weight:800;color:var(--g2)">+' + totalNetInc.toLocaleString('ru') + ' смн</div></div>' +
            '<div style="background:#ede9fe;border-radius:10px;padding:10px;text-align:center"><div style="font-size:10px;color:#5b21b6;margin-bottom:3px">Выведено</div><div style="font-size:15px;font-weight:800;color:#7C3AED">−' + totalWithdrawn.toLocaleString('ru') + ' смн</div></div>' +
        '</div>';
    } catch(e) { console.error(e); }
}
...
async function submitWithdraw() {
    ...
    try {
        ...
        loadTeacherEarnings();
    } catch(e) { errEl.textContent = e.message || 'Ошибка'; errEl.style.display = 'block'; }
    finally { btn.disabled = false; btn.textContent = 'Подать заявку на вывод'; }
}
    ...
    _approvalPollTimer = setInterval(async function() {
        try {
            ...
            if (latest && latest.status === 'approved') {
                clearInterval(_approvalPollTimer); _approvalPollTimer = null;
                var bal = await get('/payments/balance');
                currentUser.balance = bal.balance;
                localStorage.setItem('user', JSON.stringify(currentUser));
                showLoggedIn();
                localStorage.removeItem('pendingCourseId'); localStorage.removeItem('pendingProfileId');
                pendingCourseId = null; currentProfileId = null;
                showToast('🎉 ' + (latest.admin_comment || 'Оплата прошла успешно!'));
                sdShow('my-courses'); loadMyCourses();
            } else if (latest && latest.status === 'rejected') {
                clearInterval(_approvalPollTimer); _approvalPollTimer = null;
                showToast('❌ Заявка отклонена. ' + (latest.admin_comment || ''), 'error');
                sdShow('payment-flow'); initPayFlow();
            }
        } catch(e) {}
    }, 10000);
}
...
async function submitForgotPassword() {
    ...
    btn.disabled = true; btn.textContent = '⏳ Отправка...'; msg.style.display = 'none';
    try {
        await post('/auth/forgot-password', { email });
        showForgotMsg('✅ Письмо отправлено! Проверьте почту (и папку «Спам»).', 'success');
        btn.textContent = 'Отправить ещё раз';
    } catch(e) { showForgotMsg('Ошибка: ' + (e.message || ''), 'error'); btn.textContent = 'Отправить ссылку'; }
...
async function openResetPage(token) {
    _resetToken = token; go('reset');
    try {
        var result = await get('/auth/check-reset-token/' + token);
        if (!result.valid) { document.getElementById('reset-form').style.display = 'none'; document.getElementById('reset-invalid').style.display = 'block'; }
        else { document.getElementById('reset-form').style.display = 'block'; document.getElementById('reset-invalid').style.display = 'none'; document.getElementById('reset-success').style.display = 'none'; }
    } catch(e) { document.getElementById('reset-form').style.display = 'none'; document.getElementById('reset-invalid').style.display = 'block'; }
}
...
async function submitResetPassword() {
    ...
    try {
        ...
        _resetToken = null;
    } catch(e) { err.textContent = e.message || 'Ошибка.'; err.style.display = 'block'; btn.disabled = false; btn.textContent = 'Сохранить пароль'; }
}
...
const SD_LABELS = { 'terms': 'Условия', 'overview': 'Обзор', 'my-courses': 'Мои курсы', 'balance': 'Баланс', 'favorites': 'Избранное', 'chats': '💬 Сообщения', 'notifications': 'Уведомления', 'settings': 'Настройки', 'payment-flow': 'Оплата' };
const TD_LABELS = { 't-overview': 'Обзор', 't-courses': 'Мои курсы', 't-students': 'Ученики', 't-chats': '💬 Сообщения', 't-add-course': 'Добавить курс', 't-earnings': 'Доходы', 't-notifs': 'Уведомления', 't-profile': 'Профиль' };
...
document.addEventListener('click', function(e) {
    const menu = document.getElementById('mobile-menu');
    const ham  = document.getElementById('nav-ham');
    if (menu && menu.classList.contains('open') && !menu.contains(e.target) && e.target !== ham && !ham?.contains(e.target)) { closeMobileMenu(); }
});
...
function openTeacherFromCourse() {
    ...
    if (teacherId) { openProfile(teacherId); }
}
